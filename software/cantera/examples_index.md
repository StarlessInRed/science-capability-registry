...
| C03 | Cantera official Python example: Counterflow diffusion flame | counterflow diffusion flame | combustion | `software/cantera/assets/C03_counterflow_diffusion_flame.yaml` | benchmark validated | 对向扩散火焰已封装为可运行 package，覆盖无辐射/有辐射两种模式，输出火焰结构、峰值温度、火焰位置、CSV、图、日志、metrics 和 validation report。 |
| C04 | Cantera official Python example: Diffusion flame extinction strain rate | extinction strain rate | combustion | `software/cantera/assets/C04_extinction_strain_rate.yaml` | benchmark validated | 氢氧对向扩散火焰熄灭搜索已封装为可运行 package，输出 continuation history、最后燃烧解、应变率指标、CSV、图、日志、metrics 和 validation report。 |
