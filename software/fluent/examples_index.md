# Fluent Examples Index

本文件登记 Fluent 官方教程、Workbench 教程、verification manual 与 legacy case source 在能力资产中的角色。PDF 与 case package 不强制一一对应；verification manual 当前先作为 reference source，只有发现同源可运行 case package 或完成自生成同构算例后，才能进入 benchmark promotion。

## Source Packages

| source_id | logical path | source_role | observed contents | capability use |
| --- | --- | --- | --- | --- |
| `fluent_tutorial_guide_2025_r1_pdf` | `AgentKnowledge/case_library/fluent/cases/official_tutorials/Ansys_Fluent_Tutorial_Guide_2025_R1.pdf` | tutorial guide | Fluent 官方 tutorial 文档 | 理解 setup workflow、模型选项与 postprocess 语义 |
...
| `fluent_workbench_tutorial_2025_r1_pdf` | `AgentKnowledge/case_library/fluent/cases/official_tutorials/Ansys_Fluent_Workbench_Tutorial_Guide_2025_R1.pdf` | Workbench guide | Workbench tutorial 文档 | C08 problem-definition 与参数工作流来源 |
...
| `fluid_dynamics_verification_manual` | `AgentKnowledge/case_library/fluent/cases/official_tutorials/ansys_fluid_dynamics_verification_manual.pdf` | verification/reference | 未发现同步 case package | C02 reference policy 与 benchmark promotion |
...
| C07 | `software/fluent/assets/C07_heat_transfer_energy_balance.yaml` | `2d_heat_exchanger_optimizer.zip`, `effusion_cooling.zip`, battery thermal cases | compare temperature range and energy balance | case/data source-readiness passed; heat-transfer smoke pending |
| C08 | `software/fluent/assets/C08_workbench_parameter_integration.yaml` | `workbench_parameter.zip`, `workbench_elbow.zip` | compare Workbench parameter and result handoff | WBPZ static preflight passed; RunWB2 profile pending |
...
| Fluent C02 reference package | `src/science_capability_registry/fluent/verification_reference_validation/` | Converts VMFL005 pressure-drop reference into schema/config/manifest/metrics/report and runs self-generated mesh/check smoke. | `smoke` |
...
| Fluent C04 aero reference CSV parser | `src/science_capability_registry/fluent/external_aero_force_coefficients/` | Parses `fluent_aero_tutorial.zip` reference CSVs, validates ONERA Cl-vs-AoA and Cp section tables, and writes parser-only evidence. | `static-readiness` |
| Fluent C05 VOF mesh/setup manifest | `src/science_capability_registry/fluent/vof_free_surface_transient/` | Confirms official `vof.zip` is mesh-only and must not be treated as direct transient solver replay. | `static-readiness` |
| Fluent C06 sliding/rotating mesh setup manifest | `src/science_capability_registry/fluent/sliding_rotating_mesh/` | Confirms official sliding mesh and legacy single-rotating sources are mesh-only before moving-zone replay. | `static-readiness` |
| Fluent C07 heat-transfer source manifest | `src/science_capability_registry/fluent/heat_transfer_energy_balance/` | Confirms `2d_heat_exchanger_optimizer.zip` contains a matched heat-exchanger case/data pair before thermal replay. | `static-readiness` |
| Fluent C08 Workbench WBPZ preflight | `src/science_capability_registry/fluent/workbench_parameter_integration/` | Parses `workbench_parameter.zip` nested WBPZ, Workbench metadata, current parameters, and design-point log without executing Workbench. | `static-readiness` |
