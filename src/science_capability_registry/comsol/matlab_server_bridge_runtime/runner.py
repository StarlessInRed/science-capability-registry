...
def _script_text(config: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"value = mphevaluate(model, '{scalar_name}');",
            "if isnumeric(value)",
            "    scalarValue = value(1);",
            "else",
            "    scalarValue = str2double(char(value));",
            "end",
...
def _build_metrics(
    ...
    elif matlab_return_code == 0 and finite_scalar is not None:
        runtime_status = "matlab_livelink_smoke_passed"
    else:
        runtime_status = "matlab_livelink_smoke_failed"
    return {
