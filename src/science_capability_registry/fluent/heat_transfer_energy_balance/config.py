"""Configuration loading for Fluent C07 heat-transfer source readiness."""
...
SCHEMA_PATH = REPO_ROOT / "schemas" / "fluent_C07_heat_transfer_energy_balance.schema.json"
...
def validate_case_config(config: dict[str, Any], schema_path: str | Path | None = None) -> dict[str, Any]:
    ...
    if errors:
        ...
        raise ValueError("Invalid Fluent C07 config:\n" + "\n".join(messages))
    return config
