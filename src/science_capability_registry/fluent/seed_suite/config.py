"""Configuration loading for Fluent seed-suite static readiness."""
...
SCHEMA_PATH = REPO_ROOT / "schemas" / "fluent_seed_suite.schema.json"
...
def validate_suite_config(config: dict[str, Any], schema_path: str | Path | None = None) -> dict[str, Any]:
    ...
    if errors:
        ...
        raise ValueError("Invalid Fluent seed-suite config:\n" + "\n".join(messages))
    return config


def load_suite_config(path: str | Path) -> dict[str, Any]:
    return validate_suite_config(load_yaml(path))
