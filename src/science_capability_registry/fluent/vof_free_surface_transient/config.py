"""Configuration loading for Fluent C05 VOF source setup."""
...
SCHEMA_PATH = REPO_ROOT / "schemas" / "fluent_C05_vof_free_surface_transient.schema.json"
...
def validate_case_config(config: dict[str, Any], schema_path: str | Path | None = None) -> dict[str, Any]:
    ...
    if errors:
        ...
        raise ValueError("Invalid Fluent C05 config:\n" + "\n".join(messages))
    return config
