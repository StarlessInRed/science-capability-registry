"""Configuration loading for Fluent C06 sliding/rotating mesh setup."""
...
SCHEMA_PATH = REPO_ROOT / "schemas" / "fluent_C06_sliding_rotating_mesh.schema.json"
...
def validate_case_config(config: dict[str, Any], schema_path: str | Path | None = None) -> dict[str, Any]:
    ...
    if errors:
        ...
        raise ValueError("Invalid Fluent C06 config:\n" + "\n".join(messages))
    return config
