"""Configuration loading for Fluent C04 external-aero reference parsing."""
...
SCHEMA_PATH = REPO_ROOT / "schemas" / "fluent_C04_external_aero_force_coefficients.schema.json"
...
def validate_case_config(config: dict[str, Any], schema_path: str | Path | None = None) -> dict[str, Any]:
    ...
    if errors:
        ...
        raise ValueError("Invalid Fluent C04 config:\n" + "\n".join(messages))
    return config
