"""Configuration loading for Fluent official replay manifests."""
...
SCHEMA_PATH = REPO_ROOT / "schemas" / "fluent_official_replay_manifest.schema.json"
...
def validate_case_config(config: dict[str, Any], schema_path: str | Path | None = None) -> dict[str, Any]:
    ...
    if errors:
        ...
        raise ValueError("Invalid Fluent official replay manifest config:\n" + "\n".join(messages))
    return config
