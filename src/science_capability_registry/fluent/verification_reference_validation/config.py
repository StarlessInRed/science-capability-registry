"""Configuration loading for Fluent C02 verification references."""
...
SCHEMA_PATH = REPO_ROOT / "schemas" / "fluent_C02_verification_reference_validation.schema.json"
...
def validate_case_config(config: dict[str, Any], schema_path: str | Path | None = None) -> dict[str, Any]:
    ...
    if errors:
        ...
        raise ValueError("Invalid Fluent C02 config:\n" + "\n".join(messages))
    return config
