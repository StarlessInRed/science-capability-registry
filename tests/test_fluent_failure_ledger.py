...
LEDGER_PATH = Path("reports/fluent_failure_ledger.yaml")
EVIDENCE_INDEX_PATH = Path("reports/evidence_index.yaml")
LEDGER_SCHEMA_PATH = Path("schemas/fluent_failure_ledger.schema.json")
...
def test_fluent_failure_ledger_matches_schema() -> None:
...
def test_fluent_failure_ledger_structure_and_coverage() -> None:
    ledger = _read_yaml(LEDGER_PATH)

    assert ledger["ledger_id"] == "science_capability_registry.fluent_failure_ledger"
    ...
    assert ledger["scope"] == "Fluent C01-C08"
    ...
    for entry in entries:
        assert entry["failure_id"].startswith(f"FL-{entry['cxx_group']}-")
        assert entry["capability_id"].startswith("cfd.fluent.")
        ...
        assert str(entry["last_reviewed"]) == "2026-07-02"
        assert entry["evidence_refs"]
        assert entry["closure_criteria"]
        assert entry["do_not_claim"]
        for runtime_path in entry.get("runtime_evidence_paths", []):
            assert runtime_path.startswith("_results/fluent/")
            assert _repo_relative(runtime_path)


def test_fluent_failure_ledger_work_queue_resolves_entries() -> None:
    ledger = _read_yaml(LEDGER_PATH)
    ...
    for item in ledger["work_queue"]:
        ...
        for evidence_path in item["expected_evidence"]:
            assert _repo_relative(evidence_path)
    ...
    assert {
        ...
        if entry["work_queue_status"] != "resolved"
    } == set(queued_ids)
    assert "FL-C02-G002-axisymmetric-pressure-solve-open" in queued_ids
    assert "FL-C05-G001-official-vof-mesh-only-source" in queued_ids
    assert "FL-C08-G001-workbench-wbpz-not-batch-input" in queued_ids
    assert "FL-C01-F001-dimension-profile-mismatch" not in queued_ids


def test_fluent_failure_ledger_evidence_refs_resolve() -> None:
    ledger = _read_yaml(LEDGER_PATH)
    evidence_index = _read_yaml(EVIDENCE_INDEX_PATH)
    evidence_by_id = {entry["evidence_id"]: entry for entry in evidence_index["evidence"]}

    assert "fluent_failure_ledger_2026-07-02" in evidence_by_id
    ledger_evidence = evidence_by_id["fluent_failure_ledger_2026-07-02"]
        ...
        for ref in entry["evidence_refs"]:
            if ref["evidence_level"] == "repo_stable_report":
                assert ref["evidence_id"] in evidence_by_id, (
                    f"{entry['failure_id']} references missing evidence_id {ref['evidence_id']}"
                )


def test_fluent_failure_ledger_does_not_use_conversation_memory_as_evidence() -> None:
    ledger = _read_yaml(LEDGER_PATH)
