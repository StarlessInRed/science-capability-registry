...
def test_openfoam_c06_runtime_validation_requires_native_sampling_when_enabled() -> None:
    config = _baseline_config()
    config["validation"]["sampling_parity"] = {
        ...
        "evidence_id": "unit_sampling_probe",
    }
    metrics = {
        "runtime": {
            "commands": [
        ...
        "postprocess": {
            "alpha_finite": True,
            "alpha_bounds": {"alpha_min": 0.0, "alpha_max": 1.0},
            "volume": {"relative_error": 0.0},
            "front": {"front_x_m": 0.1},
            "native_sampling": {"available": False},
        },
    }
    ...
    assert result["passed"] is False
    failed = {check["name"] for check in result["checks"] if not check["passed"]}
    assert "sampling.native_sample_sets" in failed


def test_openfoam_c06_runtime_validation_requires_full_horizon_final_time() -> None:
    config = _baseline_config()
    config["validation"]["full_horizon"] = {
        ...
        "evidence_id": "unit_full_horizon_probe",
    }
    metrics = {
        "runtime": {
            "commands": [
    ...
    assert result["passed"] is False
    failed = {check["name"] for check in result["checks"] if not check["passed"]}
    assert "full_horizon.final_time" in failed
